...
class EventController extends Controller
{
    ...
    public function dashboard(Event $event)
    {
        ...
        return view('event.dashboard', ['event' => $event]);
    ...
    public function hasBookmark(Event $event)
    {
        ...
        // Check if the event is bookmarked for the authenticated user
        return $event->bookmarks()->where('user_id', auth()->id())->exists();
    }
    ...
    public function bookmarkToggle(Event $event)
    {
        // Check if the user is authenticated
        if (!auth()->check()) {
            // Handle the case where the user is not authenticated (redirect to login, show a message, etc.)
        ...
        $isBookmarked = $event->bookmarks()->where('user_id', auth()->id())->exists();

        // Return the view with the updated bookmark status
        return view('event.index', ['event' => $event, 'isBookmarked' => $isBookmarked]);
...
public function adminEdit(Event $event)
    ...
    public function adminUpdate(Request $request, Event $event)
    {
        ...
        // Redirect to the event details page
        return redirect(route('event.admin_index', ['event' => $event]))->with('success', 'Event updated successfully');
    }
