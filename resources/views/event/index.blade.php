<x-app-layout>
    <x-slot name="header" style="background-color: #75E1E1">
        <h2 class="font-semibold text-xl text-black leading-tight">
            {{ __('Events') }}
        </h2>
    </x-slot>

    <div class="py-12" style="background-image: url('{{ asset('images/background/eventdetails.png') }}'); background-size: cover; background-position: center;">
        ...
        <div class="container mx-auto px-6 sm:px-8 md:px-10 lg:px-12">
            <div class="bg-white bg-opacity-40 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 flex flex-col sm:flex-row">
                    ...
                    <div class="sm:w-1/2">
                        <img src="{{ $event['image'] ?  url('storage/' . $event['image']) : 'images/blankprofile.png'}}" alt="{{ $event->eventName }}" class="w-full h-autorounded-lg"/>
                    </div>

                    <!-- Add the bookmark button here -->
                    <form action="{{ route('event.bookmarkToggle', ['event' => $event]) }}" method="POST">
                            @csrf

                            <!-- Use a button with an image inside to make it clickable -->
                            ...
                            >
                                @if (auth()->check() && app(\App\Http\Controllers\EventController::class)->hasBookmark($event))
                                    <!-- Display bookmarked image -->
                                    <img src="{{ asset('/images/bookmark_black.png') }}" alt="Bookmarked Image"width ="30" height="40">
                                @else
                                    <!-- Display default image -->
                                    <img src="{{ asset('/images/bookmark_white.png') }}" alt="Default Image"width ="30" height="40">
                                @endif
                            </button>
                        </form>
                    ...
                    <div class="sm:w-1/2 p-4">
                        ...
                        <div id="eventClubSlider" class="relative">
                            ...
                            <div class="sm:w-1/2">
                               <img src="{{ $event->club['image'] ? url('storage/'. $event->club['image']) : 'images/blankprofile.png'}}" alt="{{ $event->club->clubname }}" class="w-32 h-auto rounded-lg" />
    ...
    <script>
        ...
        function toggleBookmark(button) {
            ...
            const statusInput = document.getElementById('bookmarkStatus');
            const eventId = statusInput.dataset.eventId; // Get the event ID from data attribute
            console.log('Toggling bookmark...');

            // Send an AJAX request to the server to check bookmark status
            fetch(`/check-bookmark/${eventId}`)
                ...
                .then(data => {
                    ...
                    if (isBookmarked) {
                        ...
                        statusInput.value = 'not-bookmarked';
